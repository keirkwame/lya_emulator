...
def generate_likelihood_class(testdir, emudir, simulation_sub_directory=None, mean_flux_label='c', max_z = 4.2, rescale_data_error=False, fix_error_ratio=False, error_ratio=100., optimise_GP=True, emulator_json_file='emulator_params.json'):
    if simulation_sub_directory is None:
        ...
        simulation_sub_directory = '/ns0.97As2.2e-09heat_slope0.05heat_amp1hub0.69/output'
    print('Beginning to initialise LikelihoodClass at', str(datetime.now()))
    return LikelihoodClass(basedir=emudir, datadir=testdir+simulation_sub_directory, mean_flux=mean_flux_label, max_z=max_z, rescale_data_error=rescale_data_error, fix_error_ratio=fix_error_ratio, error_ratio=error_ratio, optimise_GP=optimise_GP, emulator_json_file=emulator_json_file)

def run_simulations(testdir, emudir, simulation_parameters, simulation_sub_directory=None, rescale_data_error=True, optimise_GP=True):
    """Run some simulations"""
    likelihood_class_instance = generate_likelihood_class(testdir, emudir, simulation_sub_directory=simulation_sub_directory, mean_flux_label='s', rescale_data_error=rescale_data_error, optimise_GP=optimise_GP)
    likelihood_class_instance.refinement(simulation_parameters.shape[0], new_samples=simulation_parameters)
    return 1

def run_and_plot_likelihood_samples(testdir, emudir, savefile, plotname, plot_posterior=True, plot_refinement=False, chain_savedir=None, n_walkers=100, n_burn_in_steps=100, n_steps=400, while_loop=True, mean_flux_label='s', return_class_only=False, rescale_data_error=False, fix_error_ratio=False, error_ratio=100., include_emulator_error=True, max_z=4.2, emulator_json_file='emulator_params.json'):
    ...
    true_parameter_values = [0., 0.95, 0.974, 2.24e-09, 0.0509, 1.09, 0.685]

    if chain_savedir is None:
        chain_savedir = testdir
    chainfile = chain_savedir + '/AA0.97BB1.3_chain_' + plotname + '.txt'

    like = generate_likelihood_class(testdir, emudir, mean_flux_label=mean_flux_label, max_z=max_z, rescale_data_error=rescale_data_error, fix_error_ratio=fix_error_ratio, error_ratio=error_ratio, emulator_json_file=emulator_json_file)
